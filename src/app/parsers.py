...
class KzlParser(ConcertHallParser):
    ...
    def get_today_concerts(self) -> list[Concert]:
        ...
        divs = soup.find_all(HtmlPageElements.DIV.value, class_=HtmlClassNames.TODAY_1.value)
        for d in divs:
            concerts.append(
                Concert(
...
class EsseParser(ConcertHallParser):
    ...
    def get_today_concerts(self) -> list[Concert]:
        response = requests.get(self.parse_url)
        soup = BeautifulSoup(response.text, features="lxml")
        divs = soup.find_all(HtmlPageElements.ARTICLE.value, class_=HtmlClassNames.AF_ACTIVE.value)
        concerts = []
        for d in divs:
            concerts.append(
                Concert(
